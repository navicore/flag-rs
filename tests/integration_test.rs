use flag::{Command, CommandBuilder, Context, Flag, FlagType, CompletionResult};
...
fn test_dynamic_completion() {
    ...
    let pods_cmd = cmd.find_subcommand_mut("get")
    ...
    assert!(result.values.contains(&"nginx-7fb96c846b-8xvnl".to_string()));
    assert!(result.values.contains(&"nginx-7fb96c846b-kxptv".to_string()));

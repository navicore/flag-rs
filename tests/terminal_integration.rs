...
use flag_rs::terminal::{get_terminal_width, wrap_text, format_help_entry};
use std::env;
...
fn test_terminal_width_respects_columns_env() {
    ...
    for (env_value, expected) in test_cases {
        ...
        assert_eq!(
            get_terminal_width(), 
            expected,
            "COLUMNS={} should result in width={}",
            env_value,
            expected
        );
    }
    ...
    assert_eq!(get_terminal_width(), 80, "Invalid COLUMNS should fall back to 80");
...
fn test_wrap_text_preserves_content() {
    ...
    assert_eq!(original_words, wrapped_words, "Wrapping should preserve all words");
}
...
fn test_wrap_text_handles_indentation() {
    ...
    assert!(!lines[0].starts_with("    "), "First line should not be indented");
...
fn test_format_help_entry_long_flag() {
    ...
    assert_eq!(lines.len(), 2, "Long flag should push description to next line");
...
fn test_wrap_text_empty_and_edge_cases() {
    ...
    assert_eq!(wrapped, "word1 word2 word3", "Multiple spaces should be normalized");
}
...
fn visual_terminal_test() {
    ...
    let flags = vec![
        ...
        ("  --very-long-flag-name", "This flag has a long name that might affect formatting"),
    ];
